...
----------------------------------------
Runner para Transformer Ensemble (3 modelos: voyage, bge, openai).
Preserva totalmente os modelos existentes em experiment_results/transformer/.
...
Funções principais:
  ...
  learning_curve_transformer()   → lê os CSVs/modelos já salvos por % de dataset
"""
...
from ..data import load_sequence_embeddings, get_embedding_dim
...
def _make_dataloaders(sequences, labels, test_size=0.2, val_size=0.1, batch_size=32, seed=42):
    """
    ...
    idx_tr, idx_val  = train_test_split(idx_tr, test_size=val_size/(1-test_size),
                                        random_state=seed, stratify=labels[idx_tr])
...
def _train_transformer_loop(model, dl_train, dl_val, num_epochs, lr, device):
    """Treino por época com early stopping. Retorna modelo, best_state, historico."""
...
def train_transformer(
    ...
    # ── Cache check ──────────────────────────────────────────────
    if not force_retrain and ModelCache.transformer_exists(embedding, pct):
        print(f"[transformer] Cache hit: {embedding} pct={pct}")
        return load_transformer_result(embedding, dataset_name="test_internal", pct=pct)

    # ── Dados ────────────────────────────────────────────────────
    print(f"\n[transformer] Treinando: {embedding} | epochs={num_epochs} | lr={lr}")
    sequences, labels = load_sequence_embeddings("train", embedding, with_augmented=with_augmented)
    ...
    dl_train, dl_val, dl_test = _make_dataloaders(
        sequences, labels, batch_size=batch_size
    )

    # Dataset de validação externa (dataset_validation)
    sequences_val, labels_val = load_sequence_embeddings("validation", embedding, with_augmented=False)
    dl_val_ext = _make_single_dataloader(sequences_val, labels_val, batch_size=batch_size, shuffle=False)

    # ── Treino ───────────────────────────────────────────────────
    device = _get_device()
    model = TransformerScamClassifier(embedding_dim=emb_dim).to(device)
    model, best_state, df_history = _train_transformer_loop(model, dl_train, dl_val, num_epochs, lr, device)
    ...
    ModelCache.transformer_save(model, embedding, pct)
    ModelCache.save_metrics("transformer", run_id, {"test": metrics_test, "val": metrics_val})
    ModelCache.save_history("transformer", run_id, df_history)

    # Salva predições de teste interno e validação externa separadamente
    ModelCache.save_predictions("transformer", run_id, "test_internal", y_true_test, y_pred_test, y_prob_test)
    ModelCache.save_predictions("transformer", run_id, "validation", y_true_val, y_pred_val, y_prob_val)
...
def load_transformer_result(
    ...
    """
    Carregamento completo de um ExperimentResult para um embedding e dataset_name específico a partir do cache no disco.
    """
    config = {
        "strategy": "transformer",
        "embedding": embedding, "pct": pct,
        "with_augmented": with_augmented,
    }
    run_id = _run_id(config)
    emb_dim = get_embedding_dim(embedding)

    model = ModelCache.transformer_load(embedding, emb_dim, pct)
    ...
    if saved_history is None:
        saved_history = _try_load_legacy_history(embedding, pct)
    ...
    # Carrega predições salvas do disco
    y_true, y_pred, y_prob = ModelCache.load_predictions("transformer", run_id, dataset_name)

    # Se as predições ainda não existirem no cache, calcula e grava no disco
    if y_true is None:
        device = _get_device()
        model.to(device)
        if dataset_name in ["validation", "validacao_externa"]:
            seqs, lbls = load_sequence_embeddings("validation", embedding, with_augmented=False)
            dl = _make_single_dataloader(seqs, lbls, batch_size=batch_size, shuffle=False)
        else:
            seqs, lbls = load_sequence_embeddings("train", embedding, with_augmented=with_augmented)
            seq_arr = np.array(seqs, dtype=object)
            _, _, dl = _make_dataloaders(seq_arr, lbls, batch_size=batch_size)

        y_true, y_pred, y_prob = _evaluate_dl(model, dl, device)
        ModelCache.save_predictions("transformer", run_id, dataset_name, y_true, y_pred, y_prob)
...
def _try_load_legacy_history(embedding: str, pct: Optional[int]) -> Optional[pd.DataFrame]:
    _BACKEND_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    csv_path = os.path.join(_BACKEND_DIR, "experiment_results", "legacy_metrics", "historico_epocas.csv")
    if not os.path.exists(csv_path):
        return None
    try:
        df = pd.read_csv(csv_path)
        
        # Filtra por modelo (ex: Voyage, Bge, Openai)
        model_col = None
        for col in ["Modelo", "modelo", "embedding"]:
            if col in df.columns:
                model_col = col
                break
        if model_col:
            df = df[df[model_col].astype(str).str.lower() == embedding.lower()]

        # Filtra por percentual do dataset (padrão 100% se pct for None)
        target_pct = pct if pct is not None else 100
        pct_col = None
        for col in ["Percentual_Dataset", "percentual", "pct"]:
            if col in df.columns:
                pct_col = col
                break
        if pct_col:
            df = df[df[pct_col] == target_pct]

        if df.empty:
            return None

        # Renomeia colunas para o padrão padronizado em minúsculo
        rename_map = {}
        for c in df.columns:
            c_lower = c.lower()
            if c_lower == "train_loss": rename_map[c] = "train_loss"
            elif c_lower == "val_loss": rename_map[c] = "val_loss"
            elif c_lower == "train_f1": rename_map[c] = "train_f1"
            elif c_lower == "val_f1": rename_map[c] = "val_f1"
            elif c_lower == "train_acc": rename_map[c] = "train_acc"
            elif c_lower == "val_acc": rename_map[c] = "val_acc"
            elif c_lower in ["epoca", "epoch"]: rename_map[c] = "epoch"
...
def learning_curve_transformer(
    embeddings: Optional[List[str]] = None,
    percentages: Optional[List[int]] = None,
) -> ExperimentResult:
    """Lê as curvas por percentual dos experimentos salvos."""
    embeddings = embeddings or ["voyage", "bge", "openai"]
    percentages = percentages or [5, 10, 25, 50, 75, 100]

    _BACKEND_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    csv_path = os.path.join(_BACKEND_DIR, "experiment_results", "legacy_metrics", "ensemble_size.csv")
    if os.path.exists(csv_path):
        df = pd.read_csv(csv_path)
        if "percentual" in df.columns:
            df["frac"] = df["percentual"] / 100
        return ExperimentResult(
            strategy="transformer",
            config={"type": "learning_curve", "embeddings": embeddings},
            df_history=df,
        )
    ...
    for pct in percentages:
        for emb in embeddings:
            if ModelCache.transformer_exists(emb, pct):
                saved = ModelCache.load_metrics("transformer",
                    _run_id({"strategy":"transformer","embedding":emb,"pct":pct}))
                if saved:
                    saved["pct"] = pct
                    saved["embedding"] = emb
                    saved["frac"] = pct / 100
